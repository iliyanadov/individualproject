...
- **[LIM]** True for BUY (CLOB attempted first, LMSR absorbs the buy-side remainder — `hybrid-router-v2.ts:440-472`). For SELL, no LMSR fallback exists (`hybrid-router-v2.ts:462` gates on `!isSell`). "Routes between the two" is therefore asymmetric.
- **Safer wording:** "…a hybrid controller that routes buy orders between the two mechanisms according to current CLOB liquidity and inventory; sell orders run on the CLOB only." (Ch 4.6 already states this — the asymmetry should be previewed here.)
...
- **[OK]** `:462` gate `!isSell` enforces this.
...
- **[OK]** As above, plus `!isSell` gate blocks LMSR fallback.
- **[FLAG missing caveat]** A SELL with no shared YES inventory will *also* be disqualified from CLOB by the inventory check (`:526-530`), in which case the order is neither attempted on CLOB nor on LMSR. The "simply attempted on the CLOB" sentence is therefore not universally true for empty-book sells; it is true only when the trader holds YES shares.
- **Safer wording:** "A sell against an empty book is attempted on the CLOB when the trader has YES inventory, and is otherwise unfilled (since there is no LMSR sell fallback)."
